...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
	{
		...
		hButtonSum = CreateWindow(L"Button", L"Sum", WS_CHILD | WS_VISIBLE | WS_BORDER | SS_CENTER, 300, 320, 100, 20, hWnd, (HMENU)ID_SUM, hInst, 0);
		hButtonMin = CreateWindow(L"Button", L"Min", WS_CHILD | WS_VISIBLE | WS_BORDER | SS_CENTER, 300, 350, 100, 20, hWnd, (HMENU)ID_MIN, hInst, 0);
	}
    case WM_COMMAND:
        {
			...
			case ID_SUM:
			{
				...
				else
				{
					...
					std::string str = std::to_string(visitor.getValue());
					SetWindowTextA(hEditOutput, LPCSTR(str.c_str()));
				}
				break;
			}
			case ID_MIN:
			{
				...
				else
				{
					...
					list1.accept(visitor);
					std::string str = std::to_string(visitor.getMin());
					SetWindowTextA(hEditOutput, LPCSTR(str.c_str()));
				}
				break;
			}
