...
private:
	...
	private:
		...
		const List<T> & const list;
		int index;

		Iterator(const List<T> & const list2) : list(list2)
		{
			index = 0;
		}

		Iterator(const List<T> & const list2, bool f) : list(list2)
		{
			index = list.size;
		}
	...
	private:
		...
		ConstIterator(const List<T> & const list2) : list(list2)
		{
			index = 0;
		}

		ConstIterator(const List<T> & const list2, bool f) : list(list2)
		{
			index = list.size;
		}
...
template <class T>
List<T>::List<T>(const int _capacity)
{
	size = 0;
	capacity = _capacity;
	arr = new T[capacity];
...
template <class T>
List<T>::List<T>(const std::initializer_list<T> &list)
{
	size = 0;
...
template <class T>
List<T>::List<T>(List<T> &&list) : size(list.size), capacity(list.capacity), arr(list.arr)
{
	list.size = 0;
	list.arr = nullptr;
}
